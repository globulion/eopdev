...
\begin{equation}
 \Delta^{AB} E^{\rm Rep} = 
 \Delta^{AB} E^{\rm Rep}(\mathcal{O}(S)) + 
 \Delta^{AB} E^{\rm Rep}(\mathcal{O}(S^2))
\end{equation}
%
where the first\hyp{}order term with respect to the intermolecular overlap is
%
\begin{multline}
  \Delta^{AB}  E^{\rm Rep}(\mathcal{O}(S)) = -2\sum_{a\in A} \sum_{b\in B}
               S_{ab} \Big\{
           V^A_{ab} 
         + V^B_{ab} \\
 + \sum_{c\in A} \left[ 2(ab \vert cc) - (ac \vert bc) \right] 
 + \sum_{d\in B} \left[ 2(ab \vert dd) - (ad \vert bd) \right]
                \Big\}
\end{multline}
%
