...
expansion this can be written as
%
...
If the multipole series expansions as truncated
at the distributed charges $q_i$ and dipole moments ${\BM{\upmu}_i}$, that are
located at the centroids ${\bf r}_i = \tBraKet{i}{\hat{\bf r}}{i}$, 
then the interaction energy can be written approximately as
%
\begin{multline}
...
\end{equation}
%
Therefore, only overlap integrals are needed and relative distances between
atomic and LC's which leads to a great reduction of the evaluation cost
as compared to the original equation or the full multipole expansion.
...
%If localized occupied MO's are used and only distributed charges ar, then $\rho_{ki} \approx \delta_{ki}$
