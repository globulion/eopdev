...
(Figure 1) were chosen to validate the OEP\hyp{}based model of the CT energy
...
\begin{equation}\label{e:ct-pthf-ref}
 ...
 + \frac{1}{2} {\bf F}^{\rm T} \cdot {\bf D}^{-1} \cdot {\bf F} \;,
\end{equation}
%
...
whereas the remainder term is the 
approximate pure induction energy due to the polarizability effects.\cite{Li.Netzloff.Gordon.JCP.2006}
...
are the exchange and Pauli\hyp{}repulsion interaction energies, respectively. 
At the HF level of theory, their sum is in fact
...
(SAPT)
with accuracy below 0.5 kcal/mole.
...
dipole\hyp{}dipole polarizability approximation by using the computational
methodology by Li et al.,\cite{Li.Netzloff.Gordon.JCP.2006} 
in which ${\bf F}$ constitutes the electric fields aligned in a super\hyp{}vector 
according to each polarizable center, i.e. ${\bf F} = \{ {\bf F}_1, {\bf F}_2, \ldots\}$,
whereas
...
The distributed polarizabilities {\bf\textalpha} ${\boldsymbol{\mathrm{\upalpha}}}_a$ centered at the LMO centroids
