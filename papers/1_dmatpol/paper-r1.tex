...
with the heuristic knowledge of an atomistic organisation and intermolecular interaction
on a relatively large spatial scales, typically ranging beyond few nanometers. 

The advent of hybrid methods such as quantum mechanical\hyp{}molecular mechanical (QM/MM)
...
refinings.
Understanding physics that rules intermolecular interactions helped to develop
a wide family of force fields, and served a basis for embedding Hamiltonian
within an effective environment. 
...
(vibrational or vibronic solvatochromisms). 
Interface between QM and MM levels is usually realized by electrostatic embedding
and only rarely includes the Pauli exclusion principle and dispersion. 
Unfortunately, it is quite difficult to generalize the embedding potential operator
...
This Work challenges in establishing a connection bewteen the one\hyp{}particle
density and the perturbation in the environment through introducing the
density matrix susceptibility (DMS) tensors.
The associated change in the one\hyp{}particle density matrix, $\delta {\bf D}$, is postulated to be
deconvoluted into a product form of the perturbation susceptibility ${\bf B}_i$
...
molecule and could be precomputed only once. 
This could be adapted into an efficient effective 
...
is electrostatic perturbation.
It is worth 
mentioning that, at that moment, there is no simple \emph{ab initio}
method of computation of electron deformation densities without considering the entire molecular 
aggregate.\cite{Horn.Yuezhi.Head-Gordon.JCP.2016,Horn.Head-Gordon.JCP.2015}
...
in varying electric field distributions. Finally, in Section V we conclude the presentation with a few 
