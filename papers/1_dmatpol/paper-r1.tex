...
\title{Density Matrix Susceptibility Tensors} %Title of paper
...
models of the density matrix susceptibility are shown in Table~XXX. 
...
electric fields (on average) around the water molecule.
%
%for a set of varying point charge environments
%with weak, moderate and strong electric fields are shown in Table~XXX.
...
The particular density matrix susceptibilities are determined through the least\hyp{}squares 
linear regression with respect to the enseble of electrostatically petrurbed known density matrices.
...
was still at best one order of magnitude larger than the errors produced by using the
...
It is anticipated here that the above generalized models from Eq.~\eqref{e:final-model.General}
should be studied in the future with an emphasis 
on their further optimization, especially to reach RMSE below 1 kcal/mole
in strong and very strong electric fields. Owing the capability of the proposed approach to correctly capture the 
