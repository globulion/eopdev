...
%
for $w=x,y,z$. 
The superscript `$(10)$' denotes the fact that the susceptibility introduced here
...
It is instructive to start first from the discussion of the polarization under the uniform electric field
...
\begin{subequations}\label{e:exact-DMS}
 \begin{align}
  {\bf B}^{(10)}_{\alpha\beta} &= \frac{\partial D_{\alpha\beta}}{\partial {\bf F}}\Bigg|_{{\bf F}={\bf 0}} \;, \\
  {\bf B}^{(20)}_{\alpha\beta} &= \frac{1}{2} 
     \frac{\partial^2 D_{\alpha\beta}}{\partial {\bf F} \otimes \partial {\bf F}}\Bigg|_{{\bf F}={\bf 0}} \;,
 \end{align}
\end{subequations}
%
and the associated error analysis is shown in Figure~\ref{f:fig-1}.
\begin{figure}[h]
...
Electric field magnitudes in a range from 0 to 0.035 a.u. were tested in 100 random samples with random direction of electric field.
} 
\end{figure}
...
Although such linear parameterization leads to very simple, one\hyp{}step optimization problem 
from Eq.~\eqref{e:Newton} (Hessian is parameter\hyp{}independent), it
is unfortunately insufficient for an accurate description of two\hyp{}electron effects.
