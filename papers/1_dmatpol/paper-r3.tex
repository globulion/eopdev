...
in uniform and non\hyp{}uniform electric fields.$^{a,b}$}
}
\label{t:results}
\begin{ruledtabular}
\resizebox{\textwidth}{!}{%
...
\multicolumn{15}{c}{} \\ 
\multicolumn{15}{c}{\textbf{Non-Uniform Electric Field}$^{c}$} \\ 
\multicolumn{15}{c}{} \\ \hline
...
$^b$ Ranks of the density matrix susceptibility models are denoted as $R_{\rm max}$ where $R_{\rm max}$ is the maximum power in the expansion from Eq.~\eqref{e:final-model.General}.\\
$^c$ `W', `M' and `S' denote `weak', `moderate' and `strong' electric fields 
