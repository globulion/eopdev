...
are quantitatively described by the present theory in weak, moderate and even very strong electric fields (0.003 -- 0.07 a.u.).
...
at arbitrary level of theory.
%at post\hyp{}Hartree\hyp{}Fock levels of theory.
\end{abstract}
...
is $(3\times 3)$ matrix. Also, the distributed expansion centres
(labeled as $i$) are generalized to any point in the molecule.\footnote{More general
contribution due to square of electric field could be $\sum_{ij}^M {\bf B}^{(ij; 2)}_{\alpha\beta} : {\bf F}({\bf r}_i) \otimes {\bf F}({\bf r}_j)$. However, for simplicity we neglect the contributions from different sites $i$ and $j$
...
\end{subequations}
%
and consider electric field magnitudes in a range from very weak (~0.003 a.u.) to strong (0.07 a.u.).
...
Ranks of the density matrix susceptibility models are denoted as $R_{\rm max}$
where $R_{\rm max}$ is the maximum power in the expansion
from Eq.~\eqref{e:final-model.General}.
}
\label{t:results}
\begin{ruledtabular}
\resizebox{\textwidth}{!}{%
...
%\hline\hline
\toprule
\multicolumn{1}{l}{} & \multicolumn{6}{c}{\textbf{Ab Initio DMS}} &  & \multicolumn{4}{c}{\textbf{Generalized DMS}}                                                                                                  &  & \multicolumn{2}{c}{\textbf{Distributed}} \\
\cline{2-8}
\cline{9-12}
%\cline{14-15}
\textbf{}         
& \multicolumn{2}{c}{$\BM{R_{\rm max}=1}$}
& \multicolumn{2}{c}{$\BM{R_{\rm max}=1^*}$}
& \multicolumn{2}{c}{$\BM{R_{\rm max}=2^*}$}
&
& \multicolumn{2}{c}{$\BM{R_{\rm max}=1}$} 
& \multicolumn{2}{c}{$\BM{R_{\rm max}=2}$} 
&  
...
\multicolumn{1}{l}{} & \multicolumn{14}{c}{\textbf{RMSE (kcal/mol)}}              \\ %\hline
   & 4.041 & (-) & 4.823 & (-) & 0.231 & (98.40) && 5.0121 & (-) & 0.173 & (99.10) && 0.112 & (99.63) \\ 
...
\multicolumn{15}{c}{} \\ 
\multicolumn{15}{c}{\textbf{Non-Uniform Electric Field}} \\ 
\multicolumn{15}{c}{} \\ \hline
\multicolumn{1}{l}{} & \multicolumn{14}{c}{\textbf{RMSE (kcal/mol)}}   \\ 
W & 0.067 & (-)     & - & (-) & - & (-) && 0.076 & (-)     & 0.003 & (99.45) && 0.004 & (98.64)\\ 
M & 1.072 & (-)     & - & (-) & - & (-) && 1.236 & (-)     & 0.057 & (99.08) && 0.070 & (98.58)\\ 
S & 4.220 & (-)     & - & (-) & - & (-) && 5.147 & (-)     & 0.402 & (97.25) && 0.332 & (98.13) \\ \hline
...
S & 78.07 & (40.53) & - & (-) & - & (-) && 43.33 & (81.68) & 14.40 & (97.98) && 64.33 & (59.62) \\ \hline
\bottomrule
\end{tabular}%
}
\end{ruledtabular}
...
in uniform electric field.} The notation $(R=R_{\max})$ denotes the maximum rank of the electric field
...
i.e., Eq.~\eqref{e:final-model.HF} as well as Eqs.~\eqref{e:dD-Taylor} and \eqref{e:final-model.General} 
truncated at linear susceptibilities, ${\bf B}^{(1)}$,
incorrectly reproduce the polarization energy.
Interestingly, the predicted values are in noticeable correlation with the exact estimates with a slope of $\sim$2.
...
using the first\hyp{}order DMS, ${\bf B}^{(10)}$, with $R^2$ coefficients being 
greater than 97\% in case of dipole moments and grater than 80\% in case of quadrupole moments.
...
However, quadratic DMS models better predict he induced multipole moments with
%work better
...
would greatly reduce errors of the standard dipole\hyp{}dipole polarizability approach.
...
Similarly as in the case of uniform electric field, the linear DMS models, i.e.,
the \emph{ab initio} DMS model from Eq.~\eqref{e:final-model.HF} as well as
the linear DMS model, are insufficient to correctly describe the polarization energy,
although the correlation with benchmark is still very clear with a slope of $\sim2$
(Figure~\ref{f:fig-2}A). Again, induced dipole moments are reproduced very well even
...
that are able to interact with the electric field and square of electric field.
...
\begin{subequations}
 \begin{align}
  ...
  g^{[2]}_{kvw} &=-2r_{vw} \sum_N \overline{\delta D}^{(N)} F^{(N)}_{kv} F^{(N)}_{kw} \;.
 \end{align}
\end{subequations}
%
...
\begin{subequations}
 \begin{align}
  ...
  H^{[12]}_{kv,lv'w'} &= 2r_{v'w'} \sum_N F^{(N)}_{kv} F^{(N)}_{lv'} F^{(N)}_{lw'}  \;,\\
  H^{[22]}_{kvw,lv'w'} &= 2r_{vw} r_{v'w'} \sum_N F^{(N)}_{kv} F^{(N)}_{kw} F^{(N)}_{lv'} F^{(N)}_{lw'} \;.
 \end{align}
\end{subequations}
%
Note that due to the symmetry of the Hessian matrix, the block $21$
is a transpose of the block $12$. 
The composite indices $kv$ and $kvw$ are constructed from the distributed site index $k$
and the appropriate symmetry\hyp{}corrected Cartesian component 
of a particular DMS tensor ($v$ for first\hyp{}order,
and $vw$, $w<v$ for second\hyp{}order susceptibility tensor, respectively).
