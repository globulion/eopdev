...
EFPMultipolePotentialInt::EFPMultipolePotentialInt(std::vector<psi::SphericalTransform> &spherical_transforms,
 ...
 // : psi::EFPMultipolePotentialInt(spherical_transforms, bs1, bs2, deriv), max_k_(max_k) {
    : psi::OneBodyAOInt(spherical_transforms, bs1, bs2, deriv),
      ...
      max_k_(max_k) 
{
    ...
    if (deriv > 3) {
      //throw FeatureNotImplemented("LibMints", "MultipolePotentialInts called with deriv > 0", __FILE__, __LINE__);
        throw psi::PSIEXCEPTION("MultipolePotentialInts called with deriv > 3 not implemented!");
    }

    if (max_k > 3) {
      //throw FeatureNotImplemented("LibMints", "MultipolePotentialInts called with max_k > 3", __FILE__, __LINE__);
        throw psi::PSIEXCEPTION("MultipolePotentialInts called with max_k > 3 not implemented!");
    }
