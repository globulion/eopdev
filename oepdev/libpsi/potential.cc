...
PotentialInt::PotentialInt(std::vector<SphericalTransform>& st, 
                           std::shared_ptr<BasisSet> bs1, std::shared_ptr<BasisSet> bs2, 
                           std::shared_ptr<Matrix> Qxyz, int deriv)
 :   OneBodyAOInt(st, bs1, bs2, deriv)
{
    if (deriv == 0)
        potential_recur_ = new ObaraSaikaTwoCenterVIRecursion(bs1->max_am()+1, bs2->max_am()+1);
    ...
    // Setup the initial field of partial charges
    Zxyz_ = SharedMatrix(new Matrix(Qxyz));
}
 ...
 : OneBodyAOInt(st, bs1, bs2, deriv) 
{
    ...
    // Setup the initial field of partial charges
    std::shared_ptr<Matrix> Qxyz = std::make_shared<Matrix>("Probe charge (q,x,y,z)", 1, 4);
    Qxyz->set(0, 0, q);
    Qxyz->set(0, 1, x);
    Qxyz->set(0, 2, y);
    Qxyz->set(0, 3, z);
